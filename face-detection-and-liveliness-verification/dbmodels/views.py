...
from .models import Login, ALogin, User, Datim1, Datim2
import os
...
from .forms import LoginForm, UserForm, puin, puout
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user is not None:
                ...
                query1 = Datim1.objects.filter(username=username).values()
                query2 = Datim2.objects.filter(username=username).values()
                # for t1,t2 in zip(query1.values(),query2.values()):
                #     plogin = t1['plogin']
                #     plogout = t2['plogout']
                #     ft1 = plogin.strftime('%H:%M:%S')
                #     ft2 = plogout.strftime('%H:%M:%S')
                #     print(ft1-ft2)
...
def Live(request):
    ...
    def webcam_stream():
        args = {'model': 'dbmodels/liveness.model', 'le': 'dbmodels/label_encoder.pickle',
                'detector': 'dbmodels/face_detector',
